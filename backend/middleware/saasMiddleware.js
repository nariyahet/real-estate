...
const checkPropertyQuota = async (req, res, next) => {
  try {
    ...
    const tenant = await getTenantForUser(req.user.id);
    if (!tenant) {
      // If user has no organization, preserve legacy Phase 1 behavior
      return next();
    }
    ...
    if (usage.propertiesCount >= sub.max_properties) {
      return res.status(403).json({
        ...
        message: `Your organization (${tenant.organization_name}) has reached its limit of ${sub.max_properties} property listings under the ${sub.plan_name} plan. Please upgrade to add more properties.`,
        usage: {
